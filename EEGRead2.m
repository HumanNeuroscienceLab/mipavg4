function EEG=EEGRead2(file,nChannels,nPoints,sampling,uvunits,onset)
...
%       Each point is either float32 or int16
%
...
if ~getHeader
  ...
  EEG.uvunits=uvunits;
  EEG.onset=onset;
  EEG.nBins=fsize/(4*EEG.nPoints*EEG.nChannels);
...
if ~onlyHeader
    fseek(EEGfid,0,'eof');
    total_bytes = ftell(EEGfid);
    fseek(EEGfid,0,'bof');
    total_points = EEG.nPoints * EEG.nChannels * EEG.nBins;
    nbytes_datum = total_bytes / total_points;
    if(nbytes_datum == 4)
        EEG.data=fread(EEGfid,[EEG.nPoints EEG.nChannels*EEG.nBins],'float32');
    elseif (nbytes_datum == 2)
        EEG.data=fread(EEGfid,[EEG.nPoints EEG.nChannels*EEG.nBins],'int16');
    else
        fprintf('\nUnrecognized data format: %6.2f bytes per datum\n',nbytes_datum);
        error('Program cannot proceed');
    end
%    fprintf('\nNumber of bytes per EEG data point = %d\n',nbytes_datum);
